...
```
Product name:         Gnome  (silent G, pronounced "nome" — NOT "HomeIQ", NOT "Nome")
Product description:  A home operating platform that turns how well you keep your home
                      into what it's worth.
Target user:          The settled resident owner — established at their address, holding
                      the home long term, with real equity and low-grade stress about it.
                      Secondary: portfolio owners, landlords and flippers.
Value proposition:    NOT YET APPROVED — blocked on P-016.
                      Core differentiator to build from: "Managed Like an Investment,
                      Easy as a Companion."
Domain:               NOT CONFIRMED — blocked on P-002. Do not hardcode a domain anywhere.
Brand colors:         DO NOT EXIST YET — blocked on P-114. Together Agency started brand
                      design the week of Aug 17; hex codes and typography arrive in
                      "a few weeks". The 60-page brand strategy deck contains NO hex
                      codes and NO typeface names. See "Design input — current reality".
Logo file:            DOES NOT EXIST YET — blocked on P-114.
```

### The name — read this before writing a single word

The product is **Gnome**, with a G. The G is silent. It is **not** HomeIQ (that name was killed) and
it is **not** spelled "Nome" (that spelling only exists because Fathom transcribed it phonetically).

**HomeIQ was killed** because the naming brief forbids literal house words (home, house, dwell, nest,
casa, domi) and *"the intelligence suffix: IQ, genius, brain. This product is AI-enabled, not
AI-centric."* Do not reintroduce either pattern in headings, alt text, meta tags or file names.

### Copy guardrails — non-negotiable, from the brand strategy

Tone: **"Expert underneath, human on top."**

- **Warm, but not cute** — a capable friend, not a cheerful mascot. No puns, no gimmicks to seem
  likable. Warmth is earned by being useful.
- **Clear, but not simplistic** — plain language, never dumbed down. Cut jargon, not substance.
- **Reassuring, but not alarmist** — **never sell with fear.** Lower the temperature, don't raise it.
- **Smart, but not show-offy** — no buzzwords. The intelligence shows in what we catch, not in what
  we call it.

Also:
- **"The home, worth knowing" is the internal creative idea, NOT public copy.** The deck says
  explicitly: *"Not necessarily public-facing. This is for internal alignment and direction, not
  copywriting."* Do not use it as the hero headline without the PO approving it as copy.
- **"Intelligence, not storage"** is the positioning. Never describe Gnome as a place to store
  documents or as a filing cabinet — that is exactly the category cliché the brand rejects.
- **The owner is in command; we are the sidekick.** Never write copy where the product decides for
  the user.
- **Nothing may read as another spreadsheet or chore.** The dominant emotion in the category is
  low-grade dread and fatigue; the page must relieve it, not add to it.
- **No ads, ever** — do not add sponsor slots, partner logos or affiliate anything.
- **Rigor:** *"If we can't stand behind a number, we don't show it."* Only two sourced statistics
  exist (2025 Bankrate: $21,400/yr hidden cost of ownership, $8,808 of it maintenance, 42% of owners
  said bills ran higher than expected; and ~42-year median age of US housing stock, unsourced in the
  deck). Do not invent any other figure, percentage or user count.

### Design input — current reality

There is **no logo, no color palette and no typography** yet. Together Agency delivers them in a few
weeks; the landing page is dated Thursday Aug 20. So option 4 in the "Design input" section below is
the only one available, and it is incomplete.

What the deck does give you as visual direction:
- Positioning is **premium and composed, not utilitarian**. Warm carrying authority.
- The deck itself is near-black backgrounds, a warm blush/tan accent, off-white body copy, a
  high-contrast serif for display (often italic) and a neutral geometric sans for body.
- **Hard guardrail:** the deck flags that "gnome" *"risks a childish and quaint connotation. We'd
  resolve this with a brand design that leans sophisticated and premium. (Think: Robinhood)."*
  **No cartoon gnomes, no garden kitsch, no whimsy.**

Before building UI, raise this with the PO: either the page ships with a deliberate provisional
visual identity that is easy to reskin when Together Agency delivers, or the date moves. Do not
quietly invent a permanent brand.
...
```
GitHub repo:        NOT CREATED YET. The 90-day task map says the repo goes under the
                    CLIENT's GitHub org, and that feedback routes to GitHub Issues.
                    Confirm with the PO before creating anything. See P-116.
Slack channel:      #mio  (Carolina's private channel — safe to post to)
Slack channel ID:   C09SW04KRJM
Slack webhook URL:  not set — needs to be created and added to Vercel env vars
GitHub token:       not set — needs to be created and added to Vercel env vars
```

### Open blockers that affect this project

| ID | Blocker | Effect on the landing page |
|---|---|---|
| P-002 | Domain not confirmed | No domain to connect; PostHog email DNS can't be verified either |
| P-016 | Value proposition not written or approved | The hero headline has no approved source |
| P-114 | No logo, no colors, no typography from Together Agency | Cannot build final UI |
| P-117 | **PostHog was never mentioned in the client kickoff**, yet the task map assumes it as the measurement backbone | Confirm the tool before wiring events, cohorts or workflows |
| P-004 | PostHog project not created | No API key |
| P-116 | Basecamp is the client's official channel; the task map says GitHub is the tracker | Decide where feedback issues actually land before wiring the widget |
| P-020 | The landing page is dated **Thursday Aug 20** in the task map, but week 2 in the TractionLab framework, and was never mentioned in the kickoff | The date itself is unsettled |

Do not silently work around these. Surface them to the PO.

---

## Project context — the Gnome source of truth

This landing page lives inside the wider Gnome project. **Read these before writing copy or UI.**
Paths are relative to this folder:

| File | What it is |
|---|---|
| `../../context/gnome-brand-strategy.md` | **THE BRAND.** The full 60-page Together Agency deck: mission, vision, the internal creative idea, the core differentiator, the 6 values, the 5 personality traits, the tone of voice, the Know / Grow / Hand Off architecture, the value proposition mapping, and the 11 mapped competitors. This is the authority for all copy. |
| `../../context/tractionlab-metodologia.md` | The TractionLab method and the official NotebookLM Overview prompt. |
| `../../context/task-map.md` | The official 90-day plan: dates, owners, gates. Authority over any other timeline. |
| `../../context/mis-pendientes.md` | The PO's open tasks, with the P-0NN IDs referenced above. |
| `../../context/meetings/` | Processed meeting notes. The kickoff of 2026-08-17 has the client's own words on scope, monetization and platform. |
| `../../CLAUDE.md` | The parent project agent instructions. |

### Decisions already made — do not reopen these

- **The platform is a responsive WEB app.** A native app is an explicitly later phase with no date or
  scope. Confirmed twice by the client in the kickoff.
- **Monetization: no ads, ever.** A subscription priced **per home**, not per user — monthly with a
  discount for paying annually. Price still TBD. ⚠ Note the brand deck instead proposes premium
  features + AI usage tiers + paid collaborators; these are not the same model and the conflict is
  still open (P-017). Do not put pricing on the page.
- **No native social feed.** The brand strategy routes that energy two ways instead: a private
  inspiration-and-collaboration layer inside the product, and lightweight "share to" links out to
  Instagram and Pinterest.
- **Phase I modules:** Paint Manager · Equipment Manager · Financial History (cost basis) ·
  Insurance Report · Value Tracker. Only these. Do not promise contractor portals, IoT, multi-property
  or an AI interior designer on the page — all were explicitly deferred by the client.
- **Acquisition trigger:** the moment someone renovates or buys a home. Launch geography is the US
  Northeast (NY / NJ / CT), starting with friends and neighbours. No paid spend before the raise.
